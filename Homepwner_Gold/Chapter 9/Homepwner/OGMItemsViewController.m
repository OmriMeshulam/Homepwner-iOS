...
{
    return [[[OGMItemStore sharedStore] allItems] count] + 1;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // If current row == index number plus 1 for the extra row
    if (indexPath.row == [[[OGMItemStore sharedStore]allItems]count])
    {
...
// Renaming Delete button text to Remove
- (NSString *)tableView:(UITableView *)tableView titleForDeleteConfirmationButtonForRowAtIndexPath:(NSIndexPath *)indexPath
{
    return @"Remove";
}

// Rules for whether a row is moveable
...
// Checking how, from where to where a row moves
-(NSIndexPath *)tableView:(UITableView *)tableView targetIndexPathForMoveFromRowAtIndexPath:(NSIndexPath *)sourceIndexPath toProposedIndexPath:(NSIndexPath *)proposedDestinationIndexPath
{
    if (proposedDestinationIndexPath.row == [[[OGMItemStore sharedStore]allItems]count]) // if the dest is no more items
    {
        int row = proposedDestinationIndexPath.row;
        return [NSIndexPath indexPathForRow:row -1 inSection:proposedDestinationIndexPath.section]; // making it move right before (-1) could have also returned to sourceIndexPath for no movement
    }
